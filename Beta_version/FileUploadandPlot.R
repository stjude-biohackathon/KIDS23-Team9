...
ui<-(pageWithSidebar(
  ...
  (
    ...
    selectInput("plot.type","Plot Type:",
                list(boxplot = "boxplot", histogram = "histogram", density = "density", bar = "bar", scatter = "scatter", heatmap = "heatmap", qqplot ="qqplot", venn= "venn")
    ),
...
server<-(function(input, output, session){
  ...
  output$p <- renderPlot({
    ...
    plot.type<-switch(input$plot.type,
                      ...
                      "scatter" = geom_point(stat = "identity", position = "identity"),
                      "heatmap" = geom_bin2d(stat = "identity", position = "identity"),
                      
                      "venn" = geom_venn(stat = "identity", position = "identity")
    )
